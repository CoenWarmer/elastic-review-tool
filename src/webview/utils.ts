...
export const BOT_LOGINS = new Set(['coderabbitai', 'coderabbitai[bot]', 'elasticmachine']);
export const isBot = (login: string) => BOT_LOGINS.has(login) || login.endsWith('[bot]');
...
export function isReviewInProgress(pr: GhPullRequest): boolean {
  ...
  return (pr.latestReviews ?? []).some((r) => r.state !== 'PENDING' && isHuman(r.author.login));
}
...
export function cfStatusIcon(status: string): { icon: string; colorClass: string } {
  switch (status) {
    case 'added':
      return { icon: '+', colorClass: 'status-added' };
    case 'deleted':
      return { icon: '−', colorClass: 'status-deleted' };
    case 'renamed':
      return { icon: '→', colorClass: 'status-renamed' };
    default:
      return { icon: '●', colorClass: 'status-modified' };
  }
}
...
export interface CfFolderNode {
  type: 'folder';
  name: string;
  children: CfTreeChild[];
}
export interface CfFileLeaf {
  type: 'file';
  file: OrderedFile;
}
...
export const bkStateIcon = (state: string): { icon: string; cls: string } => {
  switch (state) {
    case 'passed':
      return { icon: '✓', cls: 'bk-passed' };
    case 'failed':
      return { icon: '✗', cls: 'bk-failed' };
    case 'failing':
      return { icon: '⚠', cls: 'bk-failing' };
    case 'running':
      return { icon: '⟳', cls: 'bk-running' };
    case 'blocked':
      return { icon: '⏸', cls: 'bk-blocked' };
    default:
      return { icon: '●', cls: 'bk-unknown' };
  }
};
...
export function extractBuildkiteSummary(text: string): BuildkiteSummaryItem[] {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    try {
      data = JSON.parse(m[2].trim()) as { builds: BuildkiteBuild[] };
    } catch {
      continue;
    }
...
export function renderMarkdown(text: string, repoUrl?: string): string {
  ...
  html = html.replace(
    /!\[([^\]]*)\]\((https?:\/\/[^)]+)\)/g,
    (_, alt, src) => `<img src="${src}" alt="${alt}" style="max-width:100%">`
  );
  ...
  if (repoUrl) {
    html = html.replace(
      /(^|[\s(,;])#(\d+)/gm,
      (_, pre, n) => `${pre}<a href="${repoUrl}/issues/${n}">#${n}</a>`
    );
  }
...
export function applyEmoji(text: string): string {
  return text.replace(/:([a-zA-Z0-9_+\-]+):/g, (match, name: string) => EMOJI[name] ?? match); // eslint-disable-line no-useless-escape
}
